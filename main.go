...
FIXME do replacements in comments too?
*/
		...
		for _, x := range cg.List {
			...
			if matches != nil {
				t := &templateInstantiation{
					...
					InputFile:  path.Clean(inputFile),
...
func (ti *templateInstantiation) parse(inputFile string) {
	...
	outputFilePath := path.Join(ti.Dir, outputFileName)
	outputFile(fset, newFile, outputFilePath)
	logf("Written '%s'", outputFilePath)
}
